...
Level.prototype.nextLevel = function() {
  ...
  this.smallPlaneCount = 0
  this.bossPlane = false
  this.releaseSpeed = this.releaseSpeed - 50
  releaseCounter = 0
};
...
Level.prototype.makePlanes = function() {
  let computerPlanes = []
  releaseCounter++;
  if (releaseCounter === this.releaseSpeed && this.smallPlaneCount < 2 + this.number) {
    this.smallPlaneCount++
    releaseCounter = 0;
    let smallPlane = new SmallComputerPlane({type: "computer"});
    computerPlanes.push(smallPlane);
  } else if (this.smallPlaneCount === 2 + this.number && this.bossPlane === false ) {
