...
function drawWings(xMid,context,plane) {
  context.beginPath();
...
function drawNose(xMid,context,plane) {
...
function drawComputerNose(xMid,context,plane) {
...
function drawComputerFuselageTail(xMid,context,plane) {
  context.moveTo(plane.x, plane.y + plane.height);
  context.lineTo(xMid, plane.y + plane.height + 5);
  context.lineTo(plane.x + plane.width, plane.y + plane.height);
  context.fill();
...
function drawTail(xMid,context,plane) {
...
Renderer.prototype.drawPlanes = function(planes) {
  ...
  planes.forEach(function(plane){
    ...
    if (plane.type === "player") {
      drawWings(xMid,context,plane)
      drawNose(xMid,context,plane)
      drawComputerNose(xMid,context,plane)
      drawTail(xMid,context,plane)
